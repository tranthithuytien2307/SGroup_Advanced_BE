...
class TemplateModel {
  ...
  async findByIdWithLists(templateId: number): Promise<TemplateBoard | null> {
    return this.templateBoardRepository.findOne({
      where: { id: templateId },
      relations: ["lists", "lists.cards"],
      order: {
  ...
  async getActiveTemplates(): Promise<TemplateBoard[]> {
    return this.templateBoardRepository.find({
